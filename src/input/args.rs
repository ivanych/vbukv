...
//!
//! Основной метод модуля — [Args::argparse].
//! Приложение [vbukv](../../../vbukv_cli/index.html) использует
//! именно этот способ для разбора аргументов.
//!
//! Также в модуле есть вспомогательный метод [Args::argparse_from].
//! Она не разбирает аргументы командной строки напрямую,
//! но разбирает список текстовых значений,
//! идентичный значениям, задаваемым в командной строке.
//! Этот метод нужен для отладочных целей или для построения приложения,
//! берущего параметры запуска не из аргументов командной строки, а, например,
//! из конфига.
//!
//! Оба метода возвращают структуру [Args], поля которой содержат
...
impl Args {
    ...
    /// (см. также вспомогательный метод [Args::argparse_from]).
    ///
    ...
    /// ```
    ///
    /// Это вспомогательный метод (основной — [Args::argparse]).
